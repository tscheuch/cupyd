...
def georeference_models(mf_model=None, swmm_shp=None):
    if not isinstance(mf_model, flopy.modflow.Modflow):
        raise TypeError(
            f"Attribute mf_model: {mf_model}, is not of type {flopy.modflow.Modflow}."
        )
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            entrance = [
                ...
                mf_model.drn.stress_period_data.array["elev"][0][0][row][
                    col
                ],  # drn_elev (Layer y stress period???)
                mf_model.drn.stress_period_data.array["cond"][0][0][row][
                    col
                ],  # drn_cond
            ]
